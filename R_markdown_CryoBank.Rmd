...
## Subset the data
physeq.ori <- subset_samples(physeq,!(id %in% sample.remove))
...
Constrained Quantile Normalisation (ConQUR) <https://www.nature.com/articles/s41467-022-33071-9> is a normalization technique commonly used in the analysis of high-throughput sequencing data, particularly in microbiome studies. It is a type of **quantile normalisation** approach that preserves the relative abundances of taxa between samples while simultaneously removing systematic technical variations that can arise due to differences in sequencing depth, PCR amplification bias, or other factors. ConQUR uses kernel density estimation to model the distribution of taxon abundances across all samples, and then constrains the normalisation process to maintain the relative position of each taxon within that distribution.
...
## (1) Sub-samples for Wheat and Bulk Soil groups for the comparisons
physeq.W.B <- physeq.norm %>% subset_samples(Type %in% c("Wheat", "Bulksoil"))

## (2) Subset the samples in the phyloseq object that belong to "Wheat" 
## or other crops "Barley", "Beans", "Bulksoil", "Oats", "OilseedRape", "Sugarbeet"
physeq.wheat <- subset_samples(physeq.norm, Type=="Wheat")

## (3A) Merge the replicate samples for each Group
physeq.wheat.group = merge_samples(physeq.wheat, "Group") # Sum between replicate samples

# (3B) repair factors in the sample metadata
sample_data(physeq.wheat.group)$Group <- levels(sample_data(physeq.norm)$Group)[get_variable(physeq.wheat.group, "Group")]
sample_data(physeq.wheat.group)$Soil <- levels(sample_data(physeq.norm)$Soil)[get_variable(physeq.wheat.group, "Soil")]

## (4) Further subgroup for WH.CL.BO vs WH.CL.YO
physeq.wheat.vs <- physeq.wheat %>% subset_samples(Group %in% c("WH.CL.BO", "WH.CL.YO"))
```
...
**PCoA (Principal Coordinates Analysis)** PCoA is also used to visualize and analyse beta diversity by representing samples in a lower-dimensional space. It is a type of metric multidimensional scaling. PCoA is based on a metric approach, which means it attempts to preserve the actual distances between samples as closely as possible in the lower-dimensional space. PCoA produces an ordination plot where the distances between points reflect the actual dissimilarities between samples. Unlike NMDS, PCoA may not perform as well with non-linear or rank-based dissimilarity measures.
...
```{r Bata, warning=FALSE, message=FALSE}
# method options: NMDS / PCoA
pcoa <- ordinate(physeq = physeq.W.B, method = "NMDS", distance = "bray")
...
alpha.object <- cbind(
  x = sample_data(physeq.wheat),
  y = estimate_richness(physeq.wheat, measures = 'Shannon')
)
...
for (level in genus_levels) {
  ...
  genfac <- factor(tax_table(physeq.wheat.group)[, level])
  
  # calculate the abundance of each genus within each sample
  gentab <- apply(otu_table(physeq.wheat.group), MARGIN = 1, function(x) {
...
```{r Top100_4, warning=FALSE, message=FALSE}
# Subset the taxa to Genus from physeq.wheat 
physeq.wheat.genus <- subset_taxa(physeq.wheat, Genus == "Azospirillum")

# Calculate the total abundance of Azospirillum for each sample
meta = physeq.wheat.genus@sam_data
otudf = as.data.frame(t(as.data.frame(physeq.wheat.genus@otu_table)))
...
# Clean up by removing unnecessary objects
rm(physeq.wheat.genus, meta, otudf)
```

### Plotting the top 30 taxa at family level

```{r Top100_5, warning=FALSE, message=FALSE}
## Transform normalised ASVs to proportions 
proportions = transform_sample_counts(physeq.wheat.group, function(x) 100 * x/sum(x))

##  
Top30ASVs = names(sort(taxa_sums(proportions), TRUE)[1:71])
...
plot_bar(Rgsm30, "Group", fill = "Family", title = title) + 
  coord_flip() + 
  ylab("Percentage of Sequences") + ylim(0, 30) +  theme_classic() +   
  theme(text = element_text(size=15, colour = "black"), 
        axis.ticks = element_line(colour = "black", size = 1.25),
        axis.line = element_line(colour = 'black', size = 1.25),
        axis.text.x = element_text(angle=45, hjust=1, colour = "black", size = 13),
...
metdat = as.data.frame(as.matrix(physeq.wheat@sam_data))
dat = as.data.frame(t(as.data.frame(physeq.wheat@otu_table)))
pairwise.adonis(dat, metdat$Group, sim.function = "vegdist",
                sim.method = "bray", p.adjust.m = "bonferroni",
                reduce = NULL, perm = 10000)
...
B <- aggregate_taxa(physeq.wheat.group, "Genus", verbose = TRUE)
...
D <- as_tibble(B2) %>%
  ...
  group_by(Sample) %>%
  filter(rank(desc(Abundance)) <= 100) %>% # Filter the top 100
  ungroup()
...
# Clean up by removing unnecessary objects
# rm(B, B2, D, E, binary_fun, temp_df, upset_plot, physeq.wheat.group)
```
...
physeq2 = filter_taxa(physeq.wheat.vs, function(x) mean(x) > 0.1, TRUE)
...
lef_out <- run_lefse(physeq.wheat.vs, group = "Group", 
                     norm = "CPM", 
                     # taxa_rank = "Genus",
