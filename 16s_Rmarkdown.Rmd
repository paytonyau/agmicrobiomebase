...
Here, we use NMDS for Beta diversity. 
...
P1 <- plot_ordination(physeq = physeq.ori,
                      ordination = NMDS1,
                      color = "Plate",
                      shape = "Type"
                      ) + 
...
P2 <- plot_ordination(physeq = physeq.norm,
                ordination = NMDS2,
                color = "Plate",
                shape = "Type"
                ) +
...
fig <- ggarrange(P1, P2, labels = c("A", "B"), size = 5,
                 ncol = 2, nrow = 1)
...
#### based on Type and Soil.Location on wheat and bulk soil
...
#### Calculate the number of taxa on each level and each group (Replot required)
...
Plotting the top 10 taxa at the family level provides a succinct overview of microbial composition, highlighting the most prevalent families in the sample. This approach simplifies complex microbiome data, aiding in comparative analysis across different sample groups and revealing trends and patterns in an easily interpretable manner.

provides a clear and concise overview of the microbial composition at the family level and quickly identify which families are most prevalent in the corresponding sample groups.
...
# Create a plot for alpha diversity
# pdf(file = "Fig6A_TOP10.pdf", width = 8,height = 5)
print(p)
# Close the PDF device and save the plot to a file
# dev.off()
...
# pdf(file = "Fig8_Tree_ITS.pdf", width=12,height=5)
...
# Clean up by removing unnecessary objects
# rm(B, B2, D, E, binary_fun, temp_df, upset_plot, physeq.Sugarbeet.group, upset_plot)
```
