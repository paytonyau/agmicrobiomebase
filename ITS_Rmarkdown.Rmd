...
ASV$Soil.Type <- physeq.a@sam_data$Soil.Type

#### pdf
# pdf(file = "Suppl_ASV_ITS.pdf", width = 8,height = 5)
ggplot(ASV, aes(x = Soil.Type, y = ASV,  colour = Soil.Type))+
...
plot_ordination(physeq = physeq.a,
  ...
  geom_point(aes(color = Soil.Type), alpha = 1, size = 3) +
  theme(
  ...
  ) + stat_ellipse(geom = "polygon", type="norm", alpha=0.25, aes(fill = Soil.Type)) +
...
ggplot(data = alpha.object, aes(x = x.Soil.Type, y = Shannon, color = x.Soil.Type)) + 
  scale_color_brewer(palette = "Paired") +
  theme_classic() + 
  labs(
...
physeq <- merge_samples(physeq.a, "Soil.Type", fun = sum)
gentab_levels <- list()
...
B2 <- gentab_levels %>% reduce(full_join, by = "Soil.Type")
  
# Set row names and column names
row.names(B2) <- B2$Soil.Type
B2$Soil.Type <- NULL
colnames(B2)[1:6] <- genus_levels

print(B2)
# write.csv(B2, file = "ITS/level_counts_by_group.csv", row.names = TRUE)
B2$Soil.Type = row.names(B2)
data_long <- melt(B2)
data_long <- melt(B2, id.vars = "Soil.Type", 
                  variable.name = "Level", 
                  value.name = "Taxa")
...
ggplot(data_long, aes(x = Level, y = Taxa, 
                      color = Soil.Type, group = Soil.Type)) +
  geom_line() +
  geom_point(size = 4) +
...
D <- tibble(
  Sample = rep(sample_data$Soil.Type, each = nrow(otu_table)),
