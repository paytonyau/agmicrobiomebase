...
AyBCode <- subset_samples(physeq.a, Type== "Wheat" ) ### noted that t() may required
...
# Subset the data frame to only keep rows where the row sum is 10 or greater
df2 <- df[df$row_sum >= 1, ]
df2$genfac <- NULL
df2$row_sum <- NULL

# apply function to each element of dataframe
df_binary <- apply(df2, 2, binary_fun)
